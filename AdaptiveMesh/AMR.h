...
#define USE_LAGRANGE_POLYNOMIAL // For some reason, one has bad instability (presumably from ringing)
...
__global__ void __copy_boundaries(T* __restrict__ dat_old, T* __restrict__ dat_new,
	...
	if (bd.target_idx() == 0) // allow boundary
		...
		read_idx = make_uint3(min_uint(fmaxf(read_pos.x, padding_domain) + .5f, total_size_domain - padding_domain - 2u),
			min_uint(fmaxf(read_pos.y, padding_domain) + .5f, total_size_domain - padding_domain - 2u),
			min_uint(fmaxf(read_pos.z, padding_domain) + .5f, total_size_domain - padding_domain - 2u));
...
__inline__ __device__ T __interpolate_dat(const T* dat_old, const float3& read_pos, const uint3& read_idx, const uint& read_idx_n)
{
	...
	read_00 = read_00 * (1.f - read_pos.x) + dat_old[read_idx_n + 1u] * (read_pos.x);

	T read_01 = dat_old[read_idx_n + total_size_domain];
	read_01 = read_01 * (1.f - read_pos.x) + dat_old[read_idx_n + total_size_domain + 1u] * (read_pos.x);
	read_00 = read_00 * (1.f - read_pos.y) + read_01 * (read_pos.y);

	T read_10 = dat_old[read_idx_n + total_size_domain * total_size_domain];
	read_10 = read_10 * (1.f - read_pos.x) + dat_old[read_idx_n
		+ total_size_domain * total_size_domain + 1u] * (read_pos.x);

	T read_11 = dat_old[read_idx_n + total_size_domain * total_size_domain + total_size_domain];
	read_11 = read_11 * (1.f - read_pos.x) + dat_old[read_idx_n
		+ total_size_domain * total_size_domain + total_size_domain + 1u] * (read_pos.x);
	read_10 = read_10 * (1.f - read_pos.y) + read_11 * (read_pos.y);
	return read_00 * (1.f - read_pos.z) + read_10 * (read_pos.z);
}
...
struct round_robin_threads {
	...
	void set_stream_idx(uint idx) {
		current_stream_idx = idx & 7u;
	}
...
template <class T>
__global__ void __copy_to_child(T* __restrict__ dat_old, const uint offset,
	const int child_idx, const int parent_idx) {

	uint3 idx = threadIdx + blockDim * blockIdx;
	float3 read_pos = (make_float3(idx) - padding_domain + .5f) * (1.f / size_domain) - .5f;
	read_pos = read_pos * .5f + (make_float3(offset & 1u, (offset & 2u) >> 1u, (offset & 4u) >> 2u) - .5f) * .5f;
	read_pos = (read_pos + .5f) * size_domain + padding_domain - .5f;

#ifdef USE_LAGRANGE_POLYNOMIAL
	uint3 read_idx = make_uint3(read_pos + .5f);
#else
	uint3 read_idx = make_uint3(read_pos);
#endif // USE_LAGRANGE_POLYNOMIAL
	uint read_idx_n = parent_idx * cells_domain + read_idx.x
		+ (read_idx.y + read_idx.z * total_size_domain) * total_size_domain;
	read_pos.x -= read_idx.x; read_pos.y -= read_idx.y; read_pos.z -= read_idx.z;

	dat_old[child_idx * cells_domain + idx.x + (idx.y + idx.z * total_size_domain) 
		* total_size_domain] = __interpolate_dat(dat_old, read_pos, read_idx, read_idx_n);
}

template <class T>
void copy_to_child(smart_gpu_buffer<T>& buffer, const uint offset,
	const int child_idx, const int parent_idx, cudaStream_t& stream)
{
	dim3 threads(threadsA2_v, threadsB2_v, threadsD2_v);
	dim3 blocks(total_size_domain / threads.x, total_size_domain / threads.y, total_size_domain / threads.z);
	__copy_to_child<<<blocks, threads, 0, stream>>>(buffer.gpu_buffer_ptr, offset, child_idx, parent_idx);
}

void copy_to_child(vector_field& field, const uint offset,
	const int child_idx, const int parent_idx, round_robin_threads& threads)
{
	copy_to_child(field.x, offset, child_idx, parent_idx, threads.yield_stream());
	copy_to_child(field.y, offset, child_idx, parent_idx, threads.yield_stream());
	copy_to_child(field.z, offset, child_idx, parent_idx, threads.yield_stream());
}

void copy_to_child(tensor2_field& field, const uint offset,
	const int child_idx, const int parent_idx, round_robin_threads& threads)
{
	copy_to_child(field.xx, offset, child_idx, parent_idx, threads.yield_stream());
	copy_to_child(field.xy, offset, child_idx, parent_idx, threads.yield_stream());
	copy_to_child(field.xz, offset, child_idx, parent_idx, threads.yield_stream());
	copy_to_child(field.yx, offset, child_idx, parent_idx, threads.yield_stream());
	copy_to_child(field.yy, offset, child_idx, parent_idx, threads.yield_stream());
	copy_to_child(field.yz, offset, child_idx, parent_idx, threads.yield_stream());
	copy_to_child(field.zx, offset, child_idx, parent_idx, threads.yield_stream());
	copy_to_child(field.zy, offset, child_idx, parent_idx, threads.yield_stream());
	copy_to_child(field.zz, offset, child_idx, parent_idx, threads.yield_stream());
}

void copy_to_child(tensor2_sym_field& field, const uint offset,
	const int child_idx, const int parent_idx, round_robin_threads& threads)
{
	copy_to_child(field.xx, offset, child_idx, parent_idx, threads.yield_stream());
	copy_to_child(field.xy, offset, child_idx, parent_idx, threads.yield_stream());
